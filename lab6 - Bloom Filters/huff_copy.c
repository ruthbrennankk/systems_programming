...
void huffcoder_encode(struct huffcoder * this, char input_filename[], char output_filename[])
{
	...
	unsigned char c; 
  	while( ( c = fgetc(input_file) ) != 255 ) { //read in the next character and check that it is not EOF		
		...
		//printf("%llu\n", code);
		//fprintf(output_file, "%llu", code); //prints the codes in decimal
  	}
...
int main() {
	...
	huffcoder_count(coder, "modern-prometheus-unix.txt");
	...
	//test encode
	huffcoder_encode(coder, "modern-prometheus-unix.txt", "output.txt");
